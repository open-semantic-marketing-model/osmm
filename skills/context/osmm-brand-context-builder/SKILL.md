...
**Key difference from Messaging Framework (Phase 5):** Brand Context is **durable** brand-level voice and guardrails (high-read, rarely changes). A Messaging Framework is a **per-strategy Work Product** — the message hierarchy and value framing for a specific campaign or creative effort. Durable voice lives here; campaign-specific messaging lives there and references this.
...
3. **Separate durable from campaign-specific.** If a source mixes enduring brand voice with a specific campaign's messaging, keep only the durable part here; the campaign part belongs in a Messaging Framework.
