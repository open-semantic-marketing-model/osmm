...
| I16 | Object-graph view (`GRAPH.md`) | Whole-model graph of all 27 objects (built + backlog) by category, realized vs envisioned edges + learning loop, with an inline Mermaid version; generated by `scripts/gen_object_graph.py` (committed `osmm-object-graph.svg`). |
