...
3. **Forward references use a placeholder id.** Objects are often built before
   ...
   `AUD-PLACEHOLDER-butcherbox-jesse`). The placeholder is replaced with the
...
an id self-describing (you can tell `PER-butcherbox-jesse` is a Persona without
...
| Persona | `PER-` | `persona_id` | `PER-butcherbox-jesse` |
| Audience | `AUD-` | `audience_id` | `AUD-butcherbox-lapsed` |
